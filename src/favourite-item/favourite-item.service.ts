...
export class FavouriteItemService {
  ...
  async create(
    ...
    try {
      ...
      const existingFavourite =
        await this.prismaService.favouriteItem.findUnique({
          where: {
            user_id_product_id: {
      ...
      const favouriteItem = await this.prismaService.favouriteItem.create({
        ...
        include: {
          ...
          product: {
            // Yaratilgan yozuv bilan birga mahsulot ma'lumotini qaytarish
            include: {
              images: {
                // Mahsulotning asosiy rasmini olish (agar mavjud bo'lsa)
                where: { is_main: true },
                take: 1,
              },
      ...
      throw new InternalServerErrorException("Sevimlilarga qo'shib bo'lmadi.");
    }
  }
  ...
  async findAllByUserId(userId: number): Promise<FavouriteItem[]> {
    ...
    return this.prismaService.favouriteItem.findMany({
      ...
      include: {
        product: {
          // Har bir sevimlilar yozuvi uchun mahsulot ma'lumotlarini ham qo'shish
          include: {
            images: {
              // Mahsulotning asosiy rasmini olish
              where: { is_main: true },
              take: 1,
            },
            currency: true, // Mahsulot valyutasini olish
  ...
  async remove(
    ...
    try {
      ...
      return {
        message: `IDsi ${id} bo'lgan sevimlilar yozuvi muvaffaqiyatli o'chirildi.`,
      };
    } catch (error) {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2025') {
          // O'chiriladigan yozuv topilmadi
          throw new NotFoundException(
            `IDsi ${id} bo'lgan sevimlilar yozuvi o'chirish uchun topilmadi.`,
          );
        }
      }
      console.error(
        `IDsi ${id} bo'lgan sevimlilar yozuvini o'chirishda xatolik:`,
        error,
      );
      throw new InternalServerErrorException(
        "Sevimlilar yozuvini o'chirib bo'lmadi.",
      );
    }
  }
  ...
  async removeByProductIdAndUserId(
    ...
    try {
      ...
      return {
        message: `Mahsulot (ID: ${productId}) foydalanuvchi (ID: ${userId}) sevimlilaridan muvaffaqiyatli o'chirildi.`,
      };
    } catch (error) {
      console.error(
        `Sevimlilar yozuvini (user_id: ${userId}, product_id: ${productId}) o'chirishda xatolik:`,
        error,
      );
      throw new InternalServerErrorException(
        "Sevimlilar yozuvini o'chirib bo'lmadi.",
