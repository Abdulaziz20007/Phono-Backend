...
import { IsNotEmpty, IsString, IsInt, IsBoolean, IsOptional, Matches } from "class-validator";
...
export class CreateAddressDto {
  ...
  @ApiProperty({ example: "Toshkent sh., Amir Temur ko'chasi, 1-uy", description: "To'liq manzil" })
  ...
  @Matches(/^-?([1-8]?[0-9]|[1-9]0)\.{1}\d{1,15}$/, { message: "Kenglik yaroqli formatda bo'lishi kerak." })
  lat: string;
  ...
  @Matches(/^-?((1[0-7]|[1-9])?\d|180)\.{1}\d{1,15}$/, { message: "Uzunlik yaroqli formatda bo'lishi kerak." })
  long: string;
  ...
  @ApiProperty({ example: true, description: "Manzil aktivmi?", required: false, default: true })
